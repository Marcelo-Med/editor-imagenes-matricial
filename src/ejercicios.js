...
function obtenerCanal(matriz, canal) {
  ...
  const resultado = matriz.map(fila =>
    fila.map(pixel => ({ ...pixel })) // copia profunda pixel x pixel
  );
...
function ajustarBrillo(matriz, factor) {
  ...
  // const resultado = copiarMatriz(matriz);
  const resultado = matriz.map(fila =>
    fila.map(pixel => ({ ...pixel })) // Copia profunda
  );
...
function rotar90Grados(matriz) {
  ...
  const alto = matriz.length;        // filas originales
  const ancho = matriz[0].length;    // columnas originales

  // Crear la matriz rotada: dimensiones invertidas
  const resultado = Array.from({ length: ancho }, () =>
    Array.from({ length: alto }, () => null)
  );
...
function detectarBordes(matriz, umbral = 50) {
  ...
  // Crear matriz resultado (blanco y negro)
  ...
  for (let i = 0; i < alto - 1; i++) {
    for (let j = 0; j < ancho - 1; j++) {
      const actual = grises[i][j].r;        // gris actual
      const derecha = grises[i][j + 1].r;   // vecino derecho
      const abajo   = grises[i + 1][j].r;   // vecino inferior

      // Diferencias
      const diffDerecha = Math.abs(actual - derecha);
      const diffAbajo   = Math.abs(actual - abajo);

      const diferencia = Math.max(diffDerecha, diffAbajo);

      if (diferencia > umbral) {
        // Blanco = borde detectado
